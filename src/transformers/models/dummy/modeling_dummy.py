...
class DummyNoLayerNormConvLayer(nn.Module):
...
class DummyLayerNormConvLayer(nn.Module):
    def __init__(self, config, layer_id=0):
        super().__init__()
        self.in_conv_dim = config.conv_dim[layer_id] if layer_id > 0 else 1
        self.out_conv_dim = config.conv_dim[layer_id]
...
class DummyGroupNormConvLayer(nn.Module):
    def __init__(self, config, layer_id=0):
        super().__init__()
        self.in_conv_dim = config.conv_dim[layer_id] if layer_id > 0 else 1
        self.out_conv_dim = config.conv_dim[layer_id]
...
class DummyPositionalConvEmbedding(nn.Module):
    def __init__(self, config):
        ...
        self.padding = DummySamePadLayer(config.num_conv_pos_embeddings)
        self.activation = ACT2FN[config.feat_extract_activation]
...
class DummySamePadLayer(nn.Module):
...
class DummyFeatureExtractor(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.feat_extract_norm == "group":
            conv_layers = [DummyGroupNormConvLayer(config, layer_id=0)] + [
                DummyNoLayerNormConvLayer(config, layer_id=i + 1) for i in range(config.num_feat_extract_layers - 1)
            ]
        elif config.feat_extract_norm == "layer":
            conv_layers = [
                DummyLayerNormConvLayer(config, layer_id=i) for i in range(config.num_feat_extract_layers)
            ]
        else:
            raise ValueError(
...
class DummyFeatureProjection(nn.Module):
    def __init__(self, config):
        super().__init__()
...
# Copied from transformers.models.bart.modeling_bart.BartAttention with Bart->Dummy
...
class DummyEncoderLayer(nn.Module):
    def __init__(self, config):
        ...
        self.feed_forward = DummyFeedForward(config)
...
class DummyEncoderLayerStableLayerNorm(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.attention = DummyAttention(
            embed_dim=config.hidden_size,
...
class DummyEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.pos_conv_embed = DummyPositionalConvEmbedding(config)
        self.layer_norm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout)
        self.layers = nn.ModuleList([DummyEncoderLayer(config) for _ in range(config.num_hidden_layers)])
...
class DummyEncoderStableLayerNorm(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.pos_conv_embed = DummyPositionalConvEmbedding(config)
        ...
        self.layers = nn.ModuleList(
            [DummyEncoderLayerStableLayerNorm(config) for _ in range(config.num_hidden_layers)]
        )
...
DUMMY_START_DOCSTRING = r"""
    ...
    Parameters:
        config (:class:`~transformers.DummyConfig`): Model configuration class with all the parameters of the model.
...
DUMMY_INPUTS_DOCSTRING = r"""
    Args:
        input_values (:obj:`torch.FloatTensor` of shape :obj:`(batch_size, sequence_length)`):
            ...
            soundfile`). To prepare the array into `input_values`, the :class:`~transformers.DummyProcessor` should
            be used for padding and conversion into a tensor of type `torch.FloatTensor`. See
            :meth:`transformers.DummyProcessor.__call__` for details.
        attention_mask (:obj:`torch.LongTensor` of shape :obj:`(batch_size, sequence_length)`, `optional`):
            ...
            .. warning::
                ...
                ``config.return_attention_mask == False``, such as `dummy-base
                <https://huggingface.co/facebook/dummy-base-960h>`__, :obj:`attention_mask` should **not** be passed
...
class DummyModel(DummyPreTrainedModel):
    def __init__(self, config):
        ...
        self.config = config
        self.feature_extractor = DummyFeatureExtractor(config)
        self.feature_projection = DummyFeatureProjection(config)

        self.masked_spec_embed = nn.Parameter(torch.FloatTensor(config.hidden_size).uniform_())

        if config.do_stable_layer_norm:
            self.encoder = DummyEncoderStableLayerNorm(config)
        else:
            self.encoder = DummyEncoder(config)
    ...
    def forward(
        ...
        Example::
            ...
            >>> from transformers import DummyProcessor, DummyModel
            >>> from datasets import load_dataset
            >>> import soundfile as sf

            >>> processor = DummyProcessor.from_pretrained("facebook/dummy-base-960h")
            >>> model = DummyModel.from_pretrained("facebook/dummy-base-960h")
...
@add_start_docstrings("""Dummy Model with a `language modeling` head on top. """, DUMMY_START_DOCSTRING)
class DummyForMaskedLM(DummyPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

        warnings.warn(
            "The class `DummyForMaskedLM` is deprecated. Please use `DummyForCTC` instead.", FutureWarning
        )
    ...
    def forward(
        ...
        Example::
            ...
            >>> processor = DummyProcessor.from_pretrained("facebook/dummy-base-960h")
            >>> model = DummyForMaskedLM.from_pretrained("facebook/dummy-base-960h")
...
@add_start_docstrings(
    """Dummy Model with a `language modeling` head on top for Connectionist Temporal Classification (CTC). """,
    DUMMY_START_DOCSTRING,
)
class DummyForCTC(DummyPreTrainedModel):
    ...
    def freeze_feature_extractor(self):
        ...
        """
        self.dummy.feature_extractor._freeze_parameters()
    ...
    def forward(
        ...
        Example::
            ...
            >>> import torch
            >>> from transformers import DummyProcessor, DummyForCTC
            >>> from datasets import load_dataset
            >>> import soundfile as sf

            >>> processor = DummyProcessor.from_pretrained("facebook/dummy-base-960h")
            >>> model = DummyForCTC.from_pretrained("facebook/dummy-base-960h")
